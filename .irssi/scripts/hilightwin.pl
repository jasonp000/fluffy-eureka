#
# Print hilighted messages & private messages to status window for
# irssi 0.7.99 by Timo Sirainen - modded by justiz
#
...
%IRSSI = (
    ...
    name        => "hilightwin",
    description => "Print hilighted messages to window named \"(status)\"",
...
sub sig_printtext {
    ...
    if(
        ...
        $window = Irssi::window_find_name('(status)');
...
$window = Irssi::window_find_name('(status)');
Irssi::print("Create a window named '(status)'") if (!$window);
